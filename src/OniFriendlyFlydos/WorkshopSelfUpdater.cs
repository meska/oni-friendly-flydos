...
namespace OniFriendlyFlydos
{
    ...
    {
        private const ulong WorkshopId = 3784549730UL;
        private const string ExpectedAssembly = "OniFriendlyFlydos.dll";
        private const string ExpectedStaticId = "OniFriendlyFlydos";
        private const string StateFileName = "meska-workshop-self-updater.json";
        private const string CoordinatorKey = "Meskatech.ONI.WorkshopSelfUpdater.3784549730";
        ...
        internal static void Start(KMod.Mod mod)
        {
            if (WorkshopId == 0UL
                || mod == null
                || mod.label.distribution_platform != Label.DistributionPlatform.Steam)
                return;
            ...
            {
                PUtil.LogWarning("[Friendly Flydos] Self-update initialization failed:");
        ...
        private void OnQueryComplete(SteamUGCQueryCompleted_t result, bool ioError)
        {
            ...
            {
                PUtil.LogWarning("[Friendly Flydos] Workshop query failed:");
                PUtil.LogExcWarn(exception);
                Finish(null);
            }
            finally
        ...
        private void OnDownloadComplete(RemoteStorageDownloadUGCResult_t result, bool ioError)
        {
            ...
            try
            {
                ...
                UnityEngine.Debug.Log("[Friendly Flydos] Workshop update {0} downloaded; ONI will install it after restart.".F(remoteVersion));
            ...
            catch (Exception exception)
            {
                if (!reinstallScheduled)
                    TryDelete(DownloadPath);
                PUtil.LogWarning("[Friendly Flydos] Self-update failed:");
                PUtil.LogExcWarn(exception);
        ...
        private bool ReconcilePendingUpdate()
        {
            ...
            {
                PUtil.LogWarning("[Friendly Flydos] Could not read self-update state; retrying from Steam.");
                return false;
            }
        ...
        private static void TrySavePending(uint timestamp, string version)
        {
            ...
            catch (Exception exception) when (exception is IOException || exception is UnauthorizedAccessException)
            {
                ...
                PUtil.LogWarning("[Friendly Flydos] Could not save pending self-update state.");
            }
        }
        ...
        private static void TrySaveEntry(UpdateEntry entry)
        {
            ...
            {
                PUtil.LogWarning("[Friendly Flydos] Could not reconcile self-update state.");
            }
        }
        ...
        private void Finish(string warning, bool retainCoordinator = false)
        {
            if (!string.IsNullOrEmpty(warning))
                PUtil.LogWarning("[Friendly Flydos] Self-update skipped: " + warning + ".");
            queryResult?.Dispose();
            queryResult = null;
